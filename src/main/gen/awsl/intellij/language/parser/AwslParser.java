...
public class AwslParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // HTML_STRING_TOKEN
  public static boolean html_string(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "html_string")) return false;
    if (!nextTokenIs(b, HTML_STRING_TOKEN)) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = consumeToken(b, HTML_STRING_TOKEN);
    exit_section_(b, m, HTML_STRING, r);
    return r;
  }
